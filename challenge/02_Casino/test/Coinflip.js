...
contract('Coinflip', async () => {
    it('should make a bet', async () => {
        ...
        let amount = await coinflip.amount();
        assert.equal(web3.fromWei(amount, 'ether'), 5, 'amount is not 50');
    ...
    it('should throw an error for accepting the bet with the incorrect amount',
        async () => {
        let coinflip = await Coinflip.deployed();
